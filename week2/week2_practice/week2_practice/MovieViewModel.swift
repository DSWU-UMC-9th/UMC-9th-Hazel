...
class MovieViewModel {
    ...
    let movieModel: [MovieModel] = [
        ...
        .init(movieImage: .init(.showWhite), movieName: "백설공주", movieLike: 302, movieReserCount: 3.8),
    ...
    // 이전 영화로 돌아가기
    // 단, 첫 번째 영화일 경우 마지막 영화로 전환
    public func previousMovie() {
        currnetIndex = (currnetIndex - 1 + movieModel.count) % movieModel.count
    }
    
    // 오른쪽 버튼을 눌렀을 때 다음 영화로 이동하는 함수
    public func nextMovie() {
        currnetIndex = (currnetIndex + 1) % movieModel.count
    }
}
